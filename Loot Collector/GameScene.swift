...
class GameScene: SKScene {
    ...
    func spawnNewDisc(){
        ...
        let randomX = random(min: gameArea.minX + disc.size.width/2, max: gameArea.maxX - disc.size.width/2) //Can go farthest point on the X axis
        
        let randomY = random(min: gameArea.minY + disc.size.height/2, max: gameArea.maxY - disc.size.height/2) //Can go farthest point on the Y axis
        
        disc.position = CGPoint(x: randomX, y: randomY)
        
        self.addChild(disc)
        
        disc.run(SKAction.sequence([ //a sequence needs to be made up of actions
            SKAction.scale(to: 0, duration: 4),
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) { // run when touching the screen
        ...
        let randombomb = Int.random(in: scoreNumber...scoreNumber + 6)
